      ...
      $("#result").append(`<h3>${profile.last_name},  ${profile.first_name}</h3>`);
         for (let j = 0; j < index.practices.length; j++) {
          ...
          if (website === undefined) {
            ...
            $("#result").append(`<span class="phone">` + phoneDisplay + `</span>`);
            if (practice.accepts_new_patients = true) {
              $("#result").append(`<span class="accepting">  Accepting patients!</span>`);
            } else {
              $("#result").append(`<span class="not-accepting">  NOT accepting new patients</span>`);
            }
         }
      }
    } else {
...
$(document).ready(function() {
  ...
  $("#lookup-name").submit(function(event) {
    ...
    let name = $("#name").val();
    newLookup.getData(name, displayData, error);
  });

  $("#lookup-issue").submit(function(event) {
    event.preventDefault();
    let issue = $("#issue").val();
    newLookup.getData(issue, displayData, error);
  });
